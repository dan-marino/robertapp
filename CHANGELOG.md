...
### Added
- **Shuffle Lineup with bench timing fairness** — replaced "Shuffle Order" and "Shuffle Positions" buttons with a single "Shuffle Lineup" button. Early-third batters (top ~⅓ of batting order per gender) bench in innings 1–2; late-third bench in innings 5–6; middle-third fills remaining slots. Same rules applied to the server-generated initial lineup. Undo toast available for 8 seconds after each shuffle. New `assignBenchSlots()` pure utility in `src/utils/benchTiming.ts` implements the full rule set (R8–R12a) and is shared by both client and server paths.
